const validate = () => {
    ...
    let fnpattern = /^[A-Ba-z0-9]{3,16}$/g
    let lnpattern = /^[A-Ba-z0-9]{3,16}$/g
...
let counter = 0
document.querySelector('.fname input').addEventListener('change', (e) => {
    document.querySelector('.fname p').textContent = ''
    ...
    if(result) e.target.style.border = 'green 2px solid', counter += 1
    if (counter == 6) document.querySelector('.submit').style.backgroundColor = 'green'
})
